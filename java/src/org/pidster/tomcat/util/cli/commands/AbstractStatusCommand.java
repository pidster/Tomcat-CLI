...
public abstract class AbstractStatusCommand extends AbstractJMXCommand {
    ...
    private static final String[] WEBAPP_STATES = new String[] { "stopped", "started" };
     ...
     */
    private String datasources(ObjectName server) throws IOException, RuntimeException {

        StringBuilder s = new StringBuilder();

        List<ObjectName> globalResources = query(server.getDomain() + ":type=Resource,resourcetype=Global,*");

        if (getConfig().isOptionSet("datasources") && globalResources.size() > 0) {
            s.append("\nGlobal Resources:");
            for (ObjectName global : globalResources) {
                ...
                String dsq = server.getDomain() + ":type=DataSource,name=\"" + name + "\",class=" + type;

                List<ObjectName> dataSources = query(dsq, null);
                for (ObjectName ds : dataSources) {
                    s.append(String.format("\n %-25s[init:%s, now:%s, mxA:%s, idl:%s, mnI:%s, mxI:%s]", name,
                            attribute(ds, "initialSize"), 0, attribute(ds, "numActive"), attribute(ds, "maxActive"),
                            attribute(ds, "numIdle"), attribute(ds, "minIdle"), attribute(ds, "maxIdle")));
                    if (isVerbose()) {
                        s.append("\n - ");
     ...
     * @throws JMXException
     */
    private String engines(ObjectName service) throws RuntimeException, IOException {
     ...
     * @throws JMXException
     */
    private String connectors(String engineName, ObjectName engine) throws RuntimeException, IOException {
        StringBuilder s = new StringBuilder();
        ...
        for (ObjectName connector : connectors) {
            ...
            List<ObjectName> protocolHandlers = query(engineName + ":type=ProtocolHandler,port=" + port);
            ObjectName protocolHandler = protocolHandlers.get(0);
            String name = (String) attribute(protocolHandler, "name");

            s.append(String.format("\n  Connector:%-8s [", attribute(connector, "protocol")));

            if (address == null) {
                address = "0.0.0.0";
            }

            s.append(String.format("%s::%s:%s", attribute(connector, "scheme"), address, port));

            if (isVerbose())
                s.append(String.format(", secure=%s, redirect=%s", attribute(connector, "secure"),
                        attribute(connector, "redirectPort")));

            s.append("]");

            if (super.getConfig().isOptionSet("threads")) {
                List<ObjectName> threadPools = query(engineName + ":type=ThreadPool,name=" + name);
    ...
    private String executors(List<ObjectName> executors) {
        ...
        if (executors.size() > 0) {
            ...
            for (ObjectName executor : executors) {
                ...
                int minSpareThreads = (Integer) attribute(executor, "minSpareThreads");
                ...
                int largestPoolSize = (Integer) attribute(executor, "largestPoolSize");

                s.append("\n  Executor: ");
                s.append(name);
                s.append(String.format(" [active:%d, pool:%d, init:%d, largest:%d, max:%d, spare:%d; queue:%d]",
                        activeCount, poolSize, corePoolSize, largestPoolSize, maxThreads, minSpareThreads, queueSize));
            }
        }
     ...
     * @throws IOException
     */
    private String threads(String engine, List<ObjectName> threadPools) throws IOException {
        ...
        if (threadPools.size() >= 1) {
            ...
            for (ObjectName pool : threadPools) {
                ...
                String processorQuery = engine + ":type=RequestProcessor,worker=" + name + ",name=*";

                Comparator<ObjectName> sorter = threadSorter();

                List<ObjectName> processors = query(processorQuery, null, sorter);
                ...
                if (isVerbose()) {
                    ...
                    for (ObjectName rp : processors) {
                        ...
                        rpName = rpName.replaceAll(engine + ":type=RequestProcessor,worker=" + name + ",name=", "");

                        s.append(String.format("\n   %-16s %-12s %-10s %-9s %-9s %-9s %-9s  %s", rpName,
                                attribute(rp, "bytesSent"), attribute(rp, "bytesReceived"),
                                attribute(rp, "requestCount"), attribute(rp, "errorCount"), attribute(rp, "maxTime"),
                                attribute(rp, "processingTime"), attribute(rp, "maxRequestUri")));
                    }
                }
                else {
                    ...
                    for (ObjectName rp : processors) {
                        ...
                        s.append(String.format("\n   %-16s %-12s %-10s %-9s %-9s  %s", rpName,
                                attribute(rp, "bytesSent"), attribute(rp, "bytesReceived"),
                                attribute(rp, "requestCount"), attribute(rp, "errorCount"),
     ...
     * @throws JMXException
     */
    private String hosts(String engineName, ObjectName engine) throws RuntimeException, IOException {
        StringBuilder s = new StringBuilder();

        List<ObjectName> hosts = query(engineName + ":type=Host,host=" + getConfig().getOptionValue("hostname"));
        ...
        for (ObjectName host : hosts) {
            ...
            if ((getConfig().isOptionSet("webapps") || getConfig().isOptionSet("webapp")) && (webapps.length > 0)) {
     ...
     * @throws IOException
     */
    private String webapps(String engineName, String hostname, ObjectName[] webapps) throws IOException {
        ...
        for (ObjectName webapp : webapps) {
            ...
            if ("STARTED".equalsIgnoreCase(appState)) {
                ...
                String query = engineName + ":type=Manager,path=" + path + ",host=" + hostname;

                List<ObjectName> managers = query(query);
                ObjectName manager = managers.get(0);

                activeSessions = String.valueOf(attribute(manager, "activeSessions"));
                totalSessions = String.valueOf(attribute(manager, "sessionCounter"));

                processingTime = String.valueOf(attribute(manager, "processingTime"));

                startupTime = attribute(webapp, "startupTime") + "ms";
                tldScanTime = attribute(webapp, "tldScanTime") + "ms";
                initTime = ((Long) attribute(webapp, "startupTime") + (Long) attribute(webapp, "tldScanTime")) + "ms";

            }

            if (isVerbose()) {
                s.append(String.format("%-16s %-17s %-5s %-10s %-10s %-8s %-7s %-6s %20s", docBase, path, appState,
                        activeSessions, totalSessions, processingTime, startupTime, tldScanTime, started));
            }
            else {
                s.append(String.format("%-16s %-17s %-5s %-10s %-7s %20s", docBase, path, appState, activeSessions,
                        initTime, started));
            }
